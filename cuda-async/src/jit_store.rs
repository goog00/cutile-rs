...
//!
//! Disk persistence is **opt-in**: no store is configured by default. To enable it,
//! construct a store and install it explicitly before any kernel compilation:
//!
//! ```rust,ignore
//! use cuda_async::jit_store::{set_jit_store, FileSystemJitStore};
//! set_jit_store(Some(Box::new(FileSystemJitStore::new("/path/to/cache".into())?)));
//! ```
//!
//! When no store is installed, behavior is identical to the in-memory-only cache.
...
impl FileSystemJitStore {
    ...
    /// Create a store at a conventional cache location (`~/.cache/cutile/` on
    /// Linux, `/tmp/cutile` otherwise).
    ///
    /// This is a convenience constructor only. Disk persistence is never enabled
    /// automatically: the caller must still install the returned store via
    /// [`set_jit_store`].
...
/// Configure the global JIT store. Call once at startup, before any kernel
/// compilation.
///
/// Disk persistence is opt-in: if this is never called (or called with `None`),
/// compilation uses the in-memory cache only and never touches the filesystem.
///
...
///
/// Returns `None` when disk persistence has not been explicitly enabled via
/// [`set_jit_store`].
